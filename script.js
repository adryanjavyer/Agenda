...
function buildBookingWhatsAppUrl() {
  ...
  const message = encodeURIComponent(
    `📌 Nova reserva feita pelo site DK Barber Club:
...
` +
    `WhatsApp do cliente: ${state.clientPhone}

` +
    `Status: reserva enviada para confirmação da barbearia.
...
` +
    `Se o cliente precisar cancelar, basta acessar o link acima e enviar a mensagem pelo WhatsApp.`
  );
...
function buildCancelWhatsAppUrl() {
  ...
  const message = encodeURIComponent(
    `❌ Solicitação de cancelamento de agendamento DK Barber Club:
...
` +
    `Status: cliente solicitou cancelamento pelo site.`
  );
