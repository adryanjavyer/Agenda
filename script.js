...
function readCurrentFormState() {
  ...
  if (clientPhone) state.clientPhone = clientPhone.value.trim();
}
...
function saveCurrentBooking() {
  ...
  const existingIndex = bookings.findIndex((booking) => booking.id === record.id);
...
function initEmbeddedAdminPanel() {
  ...
  function refreshAdminFromStorage() {
  ...
  window.addEventListener("storage", (event) => {
    ...
    refreshAdminFromStorage();
    renderAdmin();
    if (state.currentStep === 2) renderTimes();
  });
...
function bindBasicAdminFallbackEvents() {
  ...
  if (openAdminButton) {
    openAdminButton.addEventListener("click", (event) => {
      event.preventDefault();
      openEmbeddedAdminPanelFallback(openAdminButton.dataset.openAdminTab || "bookings");
    });
  }
