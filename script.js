...
const ADMIN_REMEMBER_LOGIN_KEY = "dkBarberRememberAdminLogin";

// Conexão com Supabase.
// 1) Crie a tabela usando o arquivo supabase-setup.sql.
// 2) Cole abaixo a URL do projeto e a anon/public key do Supabase.
// Enquanto esses campos estiverem com "COLE_AQUI", o sistema usa localStorage como reserva.
const SUPABASE_CONFIG = {
  enabled: true,
  url: "https://grtprfkcoozhrkeetlbt.supabase.co",
  anonKey: "sb_publishable_mApPhHVMrvjUsH5oqvYU0A_UUWW3irc",
  tableName: "bookings",
...
function getAdminRememberLoginPreference() {
  const stored = localStorage.getItem(ADMIN_REMEMBER_LOGIN_KEY);
  return stored === null ? true : stored === "true";
}

function setAdminRememberLoginPreference(rememberLogin) {
  localStorage.setItem(ADMIN_REMEMBER_LOGIN_KEY, rememberLogin ? "true" : "false");
}

function getSupabaseAuthStorage() {
  const getActiveStorage = () => (getAdminRememberLoginPreference() ? localStorage : sessionStorage);
...
let remoteUnavailableTimesCache = new Map();

function getUnavailableTimesCacheKey(professionalId, dateIso) {
  return `${professionalId || ""}__${dateIso || ""}`;
}
...
async function getCurrentAdminSession() {
  ...
  if (error) {
    console.warn("Não foi possível validar a sessão do administrador:", error);
    return null;
  }
...
async function signInAdmin(email, password, rememberLogin = true) {
  const client = getSupabaseClient();
  if (!client?.auth) {
    throw new Error("Configure a URL e a anon/public key do Supabase no script.js antes de usar o login.");
  }

  setAdminRememberLoginPreference(rememberLogin);

  // Garante que uma sessão antiga não fique duplicada entre localStorage e sessionStorage
  // quando o administrador alternar a opção de manter conectado.
...
function getAdminEmailFromSession(session) {
  return session?.user?.email || "Administrador";
}

async function fetchRemoteUnavailableTimes(professionalId, dateIso) {
  const client = getSupabaseClient();
  if (!client || !professionalId || !dateIso) return [];

  const { data, error } = await client.rpc("get_unavailable_times", {
    p_professional_id: professionalId,
    p_booking_date_iso: dateIso,
  });

  if (error) {
    console.warn("Não foi possível carregar horários reservados:", error);
    return [];
  }

  return (data || []).map((item) => typeof item === "string" ? item : item.time_range).filter(Boolean);
}

async function syncSelectedRemoteUnavailableTimes() {
  if (!isSupabaseConfigured() || !state?.professionalId || !state?.dayIso) return [];

  const key = getUnavailableTimesCacheKey(state.professionalId, state.dayIso);
  const times = await fetchRemoteUnavailableTimes(state.professionalId, state.dayIso);
  remoteUnavailableTimesCache.set(key, times);
...
function getCachedRemoteUnavailableTimesForSelectedDay() {
  const key = getUnavailableTimesCacheKey(state.professionalId, state.dayIso);
  return remoteUnavailableTimesCache.get(key) || [];
}
...
function toSupabaseRow(booking) {
  return {
    ...
    booking_date_iso: booking.bookingDateIso,
    date_label: booking.dateLabel || "",
    date_formatted: booking.dateFormatted || "",
    ...
    service_price_value: Number(booking.servicePriceValue) || 0,
...
function fromSupabaseRow(row) {
  return {
    ...
    bookingDateIso: row.booking_date_iso,
...
async function fetchRemoteBookings() {
  ...
  if (SUPABASE_CONFIG.requireAdminLogin && !session) return null;
...
async function syncRemoteBookingsToLocal() {
  try {
    const remoteBookings = await fetchRemoteBookings();
    if (Array.isArray(remoteBookings)) {
      setStoredBookings(remoteBookings);
      return remoteBookings;
    }
  } catch (error) {
    console.warn("Não foi possível carregar agendamentos do Supabase:", error);
  }

  return getStoredBookings();
}

async function insertRemoteBooking(booking) {
  const client = getSupabaseClient();
  if (!client) return;
...
async function updateRemoteBookingStatus(id, status) {
  const client = getSupabaseClient();
  if (!client) return;
...
function refreshBookingScreensAfterRemoteSync(bookings) {
...
async function saveCurrentBooking() {
  ...
  try {
    await insertRemoteBooking(record);
    await syncSelectedRemoteUnavailableTimes();

    const refreshedBookings = await syncRemoteBookingsToLocal();
    refreshBookingScreensAfterRemoteSync(refreshedBookings);
  } catch (error) {
    const updatedBookings = getStoredBookings().filter((booking) => booking.id !== record.id);
    setStoredBookings(updatedBookings);
    state.bookingId = "";

    if (isSupabaseConfigured()) {
      await syncSelectedRemoteUnavailableTimes();
      alert("Este horário acabou de ser reservado por outra pessoa ou não foi possível salvar online. Escolha outro horário e tente novamente.");
      throw error;
    }

    console.warn("Agendamento salvo localmente, mas não enviado ao Supabase:", error);
  }
...
function updateCurrentBookingStatus(status) {
  ...
  updateRemoteBookingStatus(state.bookingId, status)
    .then(() => syncRemoteBookingsToLocal())
    .catch((error) => console.warn("Status atualizado localmente, mas não enviado ao Supabase:", error));
}
...
function isTimeReservedByBooking(timeRange) {
...
function getTimeUnavailableReason(timeRange) {
  if (getFixedUnavailableTimesForSelectedDay().includes(getTimeStart(timeRange))) {
    return "Cliente fixo";
  }

  if (isTimeReservedByBooking(timeRange) || getCachedRemoteUnavailableTimesForSelectedDay().includes(timeRange)) {
    return "Reservado";
  }
...
function renderProfessionals() {
  ...
  document.querySelectorAll("[data-professional-id]").forEach((button) => {
    button.addEventListener("click", () => {
      ...
      syncSelectedRemoteUnavailableTimes();
    });
  });
}
...
async function goNext() {
  ...
  try {
    ...
    console.warn("Não foi possível finalizar o agendamento:", error);
...
window.openEmbeddedAdminPanel = openEmbeddedAdminPanelFallback;
...
function initEmbeddedAdminPanel() {
  ...
  const adminShell = adminOverlay?.querySelector(".embedded-admin-shell");
  const adminLoginCard = document.getElementById("adminLoginCard");
  const adminLoginForm = document.getElementById("adminLoginForm");
  const adminLoginEmail = document.getElementById("adminLoginEmail");
  const adminLoginPassword = document.getElementById("adminLoginPassword");
  const adminRememberLogin = document.getElementById("adminRememberLogin");
  const adminLoginMessage = document.getElementById("adminLoginMessage");
  const adminLogoutButton = document.getElementById("adminLogoutButton");
  const adminUserChip = document.getElementById("adminUserChip");

  if (!adminOverlay || !adminShell || !adminLoginCard || !moreMenuButton || !moreMenuDropdown || !openAdminButton) return;
  ...
  function showAdminLogin(message = "") {
    ...
    if (adminRememberLogin) {
      adminRememberLogin.checked = getAdminRememberLoginPreference();
    }
    setAdminLoginMessage(message, message ? "error" : "");
    setTimeout(() => adminLoginEmail?.focus(), 80);
  }

  function openAdminOverlayBase() {
    adminOverlay.classList.add("open");
    adminOverlay.style.display = "block";
    adminOverlay.setAttribute("aria-hidden", "false");
    document.body.classList.add("admin-open");
  ...
  function showAdminContent(session, tab = "bookings") {
    adminLoginCard.hidden = true;
    adminShell.hidden = false;
    adminUserChip.hidden = false;
    adminLogoutButton.hidden = false;
    adminUserChip.textContent = getAdminEmailFromSession(session);

    adminState.bookings = loadAdminBookings();
    renderAdmin();
    setActiveAdminTab(tab);

    return syncRemoteBookingsToLocal().then((bookings) => {
      adminState.bookings = bookings.map(normalizeAdminBooking).sort((a, b) => getBookingDateTime(a) - getBookingDateTime(b));
      renderAdmin();
    });
  }

  async function refreshAdminAuthState(tab = adminState.pendingTab || "bookings") {
    if (!isSupabaseConfigured()) {
      showAdminLogin("Configure a URL e a anon/public key do Supabase no script.js antes de acessar o painel.");
      return;
    }
    ...
    await showAdminContent(session, tab);
  }

  async function handleAdminLogin(event) {
    event.preventDefault();
    setAdminLoginMessage("Entrando...", "loading");

    try {
      const session = await signInAdmin(
        adminLoginEmail.value,
        adminLoginPassword.value,
        adminRememberLogin ? adminRememberLogin.checked : true
      );
      adminLoginPassword.value = "";
      setAdminLoginMessage("");
      await showAdminContent(session, adminState.pendingTab || "bookings");
    } catch (error) {
      setAdminLoginMessage("Não foi possível entrar. Confira o e-mail e a senha do administrador.", "error");
      console.warn("Falha no login administrativo:", error);
    }
  }
  ...
  async function handleAdminLogout() {
    ...
    adminState.bookings = [];
    renderAdmin();
    showAdminLogin("Você saiu do painel administrativo.");
  }
  ...
  function updateBookingStatus(id, status) {
    ...
    updateRemoteBookingStatus(id, status)
      .then(() => syncRemoteBookingsToLocal())
      .then((bookings) => {
        adminState.bookings = bookings.map(normalizeAdminBooking).sort((a, b) => getBookingDateTime(a) - getBookingDateTime(b));
        renderAdmin();
  ...
  function deleteBooking(id) {
    ...
    deleteRemoteBooking(id)
      .then(() => syncRemoteBookingsToLocal())
      .then((bookings) => {
        adminState.bookings = bookings.map(normalizeAdminBooking).sort((a, b) => getBookingDateTime(a) - getBookingDateTime(b));
        renderAdmin();
      })
      .catch((error) => console.warn("Agendamento excluído localmente, mas não removido do Supabase:", error));
  }
  ...
  function addManualBooking(event) {
    ...
    insertRemoteBooking(booking)
      .then(() => syncRemoteBookingsToLocal())
      .then((bookings) => {
        adminState.bookings = bookings.map(normalizeAdminBooking).sort((a, b) => getBookingDateTime(a) - getBookingDateTime(b));
        renderAdmin();
      })
      .catch((error) => console.warn("Agendamento manual salvo localmente, mas não enviado ao Supabase:", error));
  }
  ...
  function openAdminPanel(tab = "bookings") {
    closeMoreMenu();
    adminState.pendingTab = tab || "bookings";
    openAdminOverlayBase();
    refreshAdminAuthState(adminState.pendingTab);
  ...
  function bindAdminEvents() {
    ...
    openAdminButton.addEventListener("click", (event) => {
      event.preventDefault();
      openAdminPanel(openAdminButton.dataset.openAdminTab || "bookings");
    });
  ...
  client?.auth?.onAuthStateChange((_event, session) => {
    ...
    if (session) {
      showAdminContent(session, adminState.pendingTab || adminState.currentTab || "bookings");
    } else {
      showAdminLogin();
    }
  });
  ...
  window.refreshEmbeddedAdminPanel = () => refreshAdminAuthState(adminState.currentTab || adminState.pendingTab || "bookings");
}
