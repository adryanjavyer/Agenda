...
// Somente as duas primeiras telas do fluxo usam avanço automático após a escolha.
// Tela 0 = Profissional | Tela 1 = Serviço.
// A partir da tela 2, o usuário continua precisando clicar em "Continuar".
...
function autoAdvanceAfterOptionClick() {
  if (!shouldAutoAdvanceAfterOptionClick()) return;
  if (!isCurrentStepValid()) return;

  // O setTimeout permite que a opção selecionada seja renderizada antes da troca de tela.
  // Isso evita conflito visual e mantém o fluxo atual sem alterar o layout.
  setTimeout(() => {
    goNext();
  }, 0);
}
...
function goBack() {
  ...
  // ele apenas reduz a etapa atual e atualiza a tela.
  // Ele não chama a regra de avanço automático, por isso continua voltando normalmente
  // mesmo quando a tela anterior possui avanço automático por clique.
...
function openEmbeddedAdminPanelFallback() {
  ...
  const dashboardTab = adminOverlay.querySelector('[data-admin-tab="dashboard"]');
  const dashboardPanel = adminOverlay.querySelector('[data-admin-panel="dashboard"]');

  if (dashboardTab && !adminOverlay.querySelector(".admin-tab.active")) {
    dashboardTab.classList.add("active");
  }

  if (dashboardPanel && !adminOverlay.querySelector(".admin-panel.active")) {
...
function initEmbeddedAdminPanel() {
  ...
  if (!adminOverlay || !moreMenuButton || !moreMenuDropdown || !openAdminButton) return;
  ...
  const averageTicket = $("#averageTicket");
  ...
  function renderDashboard() {
    ...
    const allValid = adminState.bookings.filter(isValidRevenueBooking);
    ...
    averageTicket.textContent = adminFormatCurrency(allValid.length ? overallRevenue / allValid.length : 0);
  ...
  function setActiveAdminTab(tab) {
    ...
    tabs.forEach((button) => button.classList.toggle("active", button.dataset.adminTab === tab));
